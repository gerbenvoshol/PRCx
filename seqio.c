...
int seqfgcgify(SEQFILE *sfpout, char *entry, int entrylen)
{
	...
	if (((double) dna / (double) (seqlen - others)) >= 0.85) {
		alpha = DNA;
	} else {
		alpha = PROTEIN;
	}
...
static void start_info(INFO *info, INTSEQFILE *isfp, int flag)
{
	...
	if (flag == SEQINFO_ALL || flag == SEQINFO_ALLINFO) {
		...
		if (flag == SEQINFO_FRAGMENT) {
			info->info->isfragment = 0;
		}

		if (flag == SEQINFO_CIRCULAR) {
			info->info->iscircular = 0;
		}

		if (flag == SEQINFO_ALPHABET) {
			info->info->alphabet = UNKNOWN;
		}

		if (flag == SEQINFO_STARTPOS) {
			info->info->fragstart = UNKNOWN;
		}
	}
}
...
static int setup_field(INFO *info, int field, int len)
{
	...
	if (info->count + len + 1 >= info->size) {
		...
		if (info->info == NULL) {
			...
			if (fname_offset != -1) {
				info->info->filename = istruct + fname_offset;
			}

			if (db_offset != -1) {
				info->info->dbname = istruct + db_offset;
			}

			if (fmt_offset != -1) {
				info->info->format = istruct + fmt_offset;
			}

			if (date_offset != -1) {
				info->info->date = istruct + date_offset;
			}

			if (list_offset != -1) {
				info->info->idlist = istruct + list_offset;
			}

			if (desc_offset != -1) {
				info->info->description = istruct + desc_offset;
			}

			if (comm_offset != -1) {
				info->info->comment = istruct + comm_offset;
			}

			if (org_offset != -1) {
				info->info->organism = istruct + org_offset;
			}

			if (his_offset != -1) {
				info->info->history = istruct + his_offset;
			}
		}
	}
...
static int get_alphabet(char *s, char *end)
{
	...
	while (end > s && (isspace(*(end - 1)) || *(end - 1) == ',')) {
		end--;
	}
...
static int guessalpha(char *seq, int seqlen, int *align_out, int *truelen_out)
{
	...
	if (newlines == 0 && ((double) dna / (double) seqlen) >= 0.85) {
		if (align_out) {
